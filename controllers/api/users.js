// API for users
// this controller has various actions: get an existing user, create a new temporary user, confirm a temporary user (and make that user permanent), reset password
'use strict';
...
router.post('/prereg', function (req, res, next) { // create a new temporary user and send confirmation e-mail
	...
	User.findOne({ username: req.body.username }, function (err, user) { // get user from final user db
		...
		if (user) {
			...
			res.sendStatus(409);  // 409: Conflict - user already exists
		} else {
						...
						if (user) {
							...
							usertemp.save(function (err) { // save user to temp db
								...
								transporter.sendMail(mailOptions, function(error, response){
									...
									console.log('registration message sent: ' + response.response);
								});
...
router.get('/confmail', function (req, res, next) { // confirm registration / pw-reset (following the link from registration e-mail)
	...
	UserTemp.findOne({ 	email: uemail,
		...
		if (user) {
			...
			console.log('no user found - registration expired!');
			// no user found: reg token expired
		}
	});
});

router.post('/confreg', function (req, res, next) { // confirm registration / pw-reset and create user / chg user pw
	User.findOne({ username: req.body.username }, function (err, user) { // get user from final user db
		if (err) { return next(err); }
		if (user) {
			// user found
		...
		bcrypt.hash(req.body.password, salt, null, function (err, hash) { // hash user password (bcrypt-nodejs)
			...
			user.save(function (err) { // save user to db (just hashed password)
				...
				UserTemp.update({ 	username: user.username,
									verified: false }, 
...
router.post('/pwreset', function (req, res, next) { // reset password and send confirmation e-mail
	// first check if user is already registered (either final or pre)
	User.findOne({ username: req.body.username }, function (err, user) { // get user from final user db
		if (err) { return next(err); }
		if (user) {
			// we have found a user -> create token
			// create a new pw reset token
			var ctoken = '';
			crypto.randomBytes(48, function(err, buffer) { // this is url-save
				ctoken = buffer.toString('hex');
			});
			// see if token in db
			//console.log('token prereq: ' + ctoken);
			UserTemp.findOne({ token: ctoken }, function (err, usertmp) {
				if (err) { return next(err); }
				if (usertmp) {
					// we have found a token, try recreate (not entirely safe, might fail another time... todo...)
					crypto.randomBytes(48, function(err, buffer) { // this is url-save
					...
					usertemp.save(function (err) { // save user to temp db
						...
						// construct confirmation link
						var conflinkplain = 'http://localhost:3000/#/confmail/email/' + user.email + '/token/' + ctoken; // this link is now clickable in yahoo!!!
						var conflinkhtml = '<a href="//localhost:3000/#/confmail?email=' + user.email + '&token=' + ctoken + '">confirm registration</a>';
						...
						var mailOptions = {
							...
							text: conflinkplain // plaintext body
							//html: conflinkhtml // html body; see https://stackoverflow.com/questions/17650174/node-mailer-cant-click-link-in-yahoo-mail
							//html: <b>PW-Reset Confirmation ✔</b><br />
							//	+ '<a href=\"' + conflinkplain.toString() + '\">confirm PW-reset</a>'
						};
						...
						transporter.sendMail(mailOptions, function(error, response){
							...
							console.log('PW-reset message sent: ' + response.response);
						});
			...
			// no user found in final collection -> error: please register
			res.sendStatus(409);  // 409: Conflict - user already exists
		}
	});
});
